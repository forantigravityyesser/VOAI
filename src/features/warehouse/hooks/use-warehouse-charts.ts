...
export function useWarehouseCharts(isMounted: boolean, data: { deadStock: ChartDataPoint[] }): {
  ...
  const deadStockOption = useMemo(() => {
    if (!isMounted) return emptyOption;
    return {
      ...
      xAxis: {
        ...
        data: data.deadStock.map((d: ChartDataPoint) => d.name || ''),
      ...
      yAxis: { 
        type: 'value' as const, 
        max: Math.max(...data.deadStock.map((d: ChartDataPoint) => (typeof d.value === 'number' ? d.value : d.value[0]))) + 200, 
        splitLine: { lineStyle: { color: 'rgba(255,255,255,0.02)', type: 'dashed' as const } }, 
      ...
      series: [{
        type: 'bar' as const, barWidth: '35%', showBackground: true,
        ...
        data: data.deadStock.map((d: ChartDataPoint) => ({
          value: d.value,
          speed: d.speed,
  ...
  const bubbleOption = useMemo(() => {
    ...
    return {
          ...
          return `
            <div style="font-family: Inter, sans-serif; min-width: 180px;">
              ...
              <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
                ...
                <b style="${(itemData.trend || '').includes('-') ? 'color: #ff4757' : 'color: #00d68f'}; font-size: 11px;">${itemData.trend || '0%'}</b>
              </div>
              <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
      ...
      xAxis: {
        ...
        nameGap: 35,
        nameTextStyle: { color: '#687092', fontSize: 10, fontWeight: 800 },
        axisLine: { lineStyle: { color: 'rgba(255,255,255,0.05)' } },
        splitLine: { show: false },
        axisLabel: { color: '#3d4567', fontWeight: 'bold' as const },
        min: 0,
        max: 100
      },
      yAxis: {
      ...
      series: [
        {
          ...
          markArea: {
            ...
            data: [
              [
                { name: 'ЛИДЕРЫ (ВЫСОКИЙ СТОК)', xAxis: 0, yAxis: 15, itemStyle: { color: '#00d68f' }, label: { show: true, position: 'insideTopLeft' as const, distance: 20, color: 'rgba(0,214,143,0.5)', fontSize: 10, fontWeight: 900 } },
                { xAxis: 50, yAxis: 30 }
              ],
              [
                { name: 'НИЗКИЙ СПРОС / ПОДДЕРЖКА', xAxis: 0, yAxis: 0, itemStyle: { color: '#00b8d9' }, label: { show: true, position: 'insideBottomLeft' as const, distance: 20, color: 'rgba(0,184,217,0.5)', fontSize: 10, fontWeight: 900 } },
                { xAxis: 50, yAxis: 15 }
              ],
              [
                { name: 'КРИТИЧЕСКИЙ ДЕФИЦИТ', xAxis: 50, yAxis: 15, itemStyle: { color: '#ff4757' }, label: { show: true, position: 'insideTopRight' as const, distance: 20, color: 'rgba(255,71,87,0.5)', fontSize: 10, fontWeight: 900 } },
                { xAxis: 100, yAxis: 30 }
              ],
              [
                { name: 'НЕЛИКВИД ПРИ ДЕФИЦИТЕ', xAxis: 50, yAxis: 0, itemStyle: { color: '#706fd3' }, label: { show: true, position: 'insideBottomRight' as const, distance: 20, color: 'rgba(112,111,211,0.5)', fontSize: 10, fontWeight: 900 } },
                { xAxis: 100, yAxis: 15 }
              ]
      ...
      series: [
        {
          ...
          detail: {
            ...
            rich: {
              ...
              label: { fontSize: 10, fontWeight: 800, color: '#687092' }
            }
          },
          data: [{ value: 72 }]
        }
  ...
  const spaceEfficiencyOption = useMemo(() => {
    ...
    return {
      ...
      xAxis: {
        ...
        nameLocation: 'center' as const,
        nameGap: 35,
        nameTextStyle: { color: '#687092', fontSize: 10, fontWeight: 900 },
        axisLine: { lineStyle: { color: 'rgba(255,255,255,0.05)' } },
        splitLine: { show: false },
        axisLabel: { color: '#3d4567', fontSize: 10 }
      },
      ...
      series: [{
        ...
        markArea: {
          ...
          data: [
            [
              { name: 'КРИТИЧЕСКИ НЕЭФФЕКТИВНО', xAxis: 20, yAxis: 0, itemStyle: { color: '#ff4757' }, label: { show: true, position: 'insideBottomRight' as const, color: 'rgba(255,71,87,0.4)', fontSize: 9, fontWeight: 900 } },
              { xAxis: 50, yAxis: 10 }
            ] 
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ] as any
