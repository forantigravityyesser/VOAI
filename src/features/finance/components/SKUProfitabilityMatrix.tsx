...
import { SKU, Segment } from "@/core/types/common";
import { mockSKUs } from "../mock-data/finance.data";
...
export default function SKUProfitabilityMatrix() {
  ...
  const filteredData = useMemo(() => 
    activeSegment === "All" ? mockSKUs : mockSKUs.filter(s => s.segment === activeSegment),
    [activeSegment]
  );
  ...
  const chartOption = {
    ...
    tooltip: {
      ...
      formatter: (params: { data: (number | SKU)[] }) => {
        const item = params.data[3] as SKU;
    ...
    grid: { left: '5%', right: '10%', bottom: '15%', top: '10%', containLabel: true },
    xAxis: {
    ...
    series: [
      {
        type: 'scatter',
        symbolSize: (data: [number, number, number, SKU]) => Math.sqrt(Math.abs(data[2])) / 2.2,
        data: filteredData.map(s => [s.revenue, s.margin, s.profit, s]),
        itemStyle: {
          color: (params: { data: [number, number, number, SKU] }) => {
        ...
        markArea: {
          ...
          data: [
            [{ name: 'Продвигать', xAxis: 0, yAxis: 25, itemStyle: { color: '#6c5ce7' } }, { xAxis: 100000, yAxis: 100 }],
            [{ name: 'Масштабировать', xAxis: 100000, yAxis: 25, itemStyle: { color: '#00d68f' } }, { xAxis: 300000, yAxis: 100 }],
            [{ name: 'Ликвидировать', xAxis: 0, yAxis: -10, itemStyle: { color: '#ff4757' } }, { xAxis: 100000, yAxis: 25 }],
            [{ name: 'Оптимизировать', xAxis: 100000, yAxis: -10, itemStyle: { color: '#ff9f43' } }, { xAxis: 300000, yAxis: 25 }]
          ]
        },
        markLine: {
          silent: true,
          ...
          data: [{ xAxis: 100000 }, { yAxis: 25 }]
        }
      }
    ]
  };

  const segmentButtons: { id: Segment; label: string }[] = [
    { id: "All", label: "Все" },
      ...
      <Card variant="glass" className="p-8">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
               <div>
                  <div className="flex items-center gap-3 mb-2">
                     ...
                     <Badge variant="info">Юнит-Экономика</Badge>
                  </div>
         ...
         <CardContent className="p-0">
            <div className="relative h-[600px] w-full bg-dark-950/40 rounded-[2rem] border border-white/5 p-4 mb-4">
               ...
               <div className="absolute top-6 left-10 pointer-events-none">
                  <div className="flex items-center gap-2 text-[#6c5ce7] bg-dark-900/40 px-3 py-1.5 rounded-lg backdrop-blur-sm border border-[#6c5ce7]/20">
                     ...
                     <span className="text-[11px] font-black uppercase tracking-widest">Продвигать</span>
                  </div>
               </div>
               <div className="absolute top-6 right-10 pointer-events-none">
                  <div className="flex items-center gap-2 text-[#00d68f] bg-dark-900/40 px-3 py-1.5 rounded-lg backdrop-blur-sm border border-[#00d68f]/20">
                     ...
                     <span className="text-[11px] font-black uppercase tracking-widest">Масштабировать</span>
                  </div>
               </div>
               <div className="absolute bottom-16 left-10 pointer-events-none">
                  <div className="flex items-center gap-2 text-[#ff4757] bg-dark-900/40 px-3 py-1.5 rounded-lg backdrop-blur-sm border border-[#ff4757]/20">
                     ...
                     <span className="text-[11px] font-black uppercase tracking-widest">Ликвидировать</span>
                  </div>
               </div>
               <div className="absolute bottom-16 right-10 pointer-events-none">
                  <div className="flex items-center gap-2 text-[#ff9f43] bg-dark-900/40 px-3 py-1.5 rounded-lg backdrop-blur-sm border border-[#ff9f43]/20">
                     ...
                     <span className="text-[11px] font-black uppercase tracking-widest">Оптимизировать</span>
                  </div>
               </div>
            </div>
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-4 gap-6">
               {[
                 { label: "Масштаб", color: "bg-accent-green", desc: "Макс. прибыль. Вливать в рекламу." },
                 { label: "Продвигать", color: "bg-accent-purple", desc: "Хорошая маржа, мало продаж. Растить охват." },
                 { label: "Оптимизировать", color: "bg-accent-orange", desc: "Много продаж, мало прибыли. Снижать косты." },
                 { label: "Ликвидировать", color: "bg-accent-red", desc: "Убыточные позиции. Выводить из оборота." }
               ].map((item, i) => (
            ...
            <div className="mt-6 text-center">
               <p className="text-[9px] font-black text-dark-300 uppercase tracking-[0.3em]">
      ...
      <div className="bg-accent-blue/10 border border-accent-blue/30 p-6 rounded-[2rem] flex items-center gap-6 group hover:bg-accent-blue/20 transition-all cursor-default relative overflow-hidden">
        ...
        <div className="flex-1 relative z-10">
           ...
           <p className="text-[13px] text-dark-200 font-medium italic leading-relaxed">
             «63% вашей выручки генерируют товары из категории <span className="text-accent-orange font-black">Optimize</span>. 
