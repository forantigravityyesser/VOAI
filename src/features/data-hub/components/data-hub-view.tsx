...
import { 
  ChevronDown, TrendingUp, Triangle, Lightbulb, 
  AlertCircle, FileText, ArrowRight, Target, LucideIcon
} from "lucide-react";
        ...
        <div className="mb-12">
          ...
          <div className="space-y-4">
            {insights.map((insight) => (
              <div 
                key={insight.id}
                className={`group flex items-start gap-4 p-5 rounded-2xl transition-all border-l-4 relative overflow-hidden
                  ${insight.type === "warning" 
                    ? "bg-orange-500/5 border-orange-500/20 hover:bg-orange-500/10 border-l-orange-500" 
                    : "bg-accent-green/5 border-accent-green/20 hover:bg-accent-green/10 border-l-accent-green"}`}
              >
                <div className={`mt-1 p-1 rounded-lg ${insight.type === "warning" ? "bg-orange-500/20" : "bg-accent-green/20"}`}>
                  {insight.type === "warning" ? (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${insight.type === "warning" ? "text-orange-500" : "text-accent-green"}`}>
                      {insight.category}
                    </span>
                    <span className="w-1 h-1 bg-dark-500 rounded-full" />
                    <span className="text-[10px] text-dark-400">{insight.status}</span>
                  </div>
                  <p 
                    className="text-sm text-dark-100 leading-relaxed"
                    dangerouslySetInnerHTML={{ __html: insight.content }}
                  />
                </div>
                <button className="opacity-0 group-hover:opacity-100 transition-opacity p-2 text-dark-400 hover:text-white">
                  {insight.type === "warning" ? <FileText className="w-4 h-4" /> : <TrendingUp className="w-4 h-4" />}
                </button>
              </div>
            ))}
          </div>
